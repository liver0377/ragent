...
> 注意：当前版本实际使用 PostgreSQL + pgvector 作为向量存储引擎，Milvus 配置项保留用于未来迁移。
...
- **Redis** — 开发环境：单实例；生产环境：Sentinel 哨兵模式
- **RabbitMQ** — 开发环境：单节点；生产环境：镜像队列集群
- **Milvus** — 开发环境：可选（用 pgvector）；生产环境：Standalone / Cluster
- **向量存储** — 开发环境：pgvector；生产环境：Milvus（推荐）
