...
/** 获取会话详情（含消息） */
export async function getConversation(convId: number | string): Promise<ConversationDetail> {
  const res = await client.get(`/conversations/${convId}`);
  return res.data.data;
}

/** 删除会话 */
export async function deleteConversation(convId: number | string): Promise<void> {
